 ...
 * @author OSC-SRL
 *
 * Convierte un numero entero a letras
 */
public class n2tDouble {
	private int flag;
	...
	public n2tDouble(double n) {
		numero = n;
		flag = 0;
	}

	private String unidad(double numeroOrig) {
		
		int numero = (int)(numeroOrig);
		switch (numero) {
		case 9:
			num = "nueve";
			break;
		...
		//NO BORRAR--- PROXIMA IMPLEMENTACION PARA CENTAVOS
		/*if(numeroOrig%1>0){
		//	decena(numero / 1000).concat(" mil ").concat(miles(numero % 1000));
			format.setMaximumFractionDigits(2);
			snumero= format.format(numeroOrig%1*100);
			snumero = snumero.replace(",", ".");
			System.out.println("snumero: "+ snumero);
			double resto = Double.parseDouble(snumero);  			
			num += " con "+ decena(resto).concat(" centavos");
		}*/
		return num;
	}

	private String decena(double numero) {
		/*Double d = new Double(132);
		Long x = new Long (d.doubleValue());*/
		numero = (int)(numero);
	...
	private String centena(double numero) {
		numero = (long)(numero);
		if (numero >= 100) {
	...
	private String miles(double numero) {
		numero = (long)(numero);
	...
	private String decmiles(double numero) {
		numero = (long)(numero);
		if (numero == 10000)
	...
	private String cienmiles(double numero) {
		numero = (long)(numero);
		if (numero == 100000)
	...
	private String millon(double numero) {
		numero = (long)(numero);
	...
	private String decmillon(double numero) {
		numero = (long)(numero);
		if (numero == 10000000)
	...
	private String cenmillon(double numero) {
		...
		if (numero > 100000000 && numero < 200000000) {
			flag = 1;
			num_letradmm = millon(numero / 1000000).concat(" millones ").concat(cienmiles(numero % 1000000));
		}
		if (numero >= 200000000 && numero < 1000000000) {
			flag = 1;
			num_letradmm = millon(numero / 1000000).concat(" millones ").concat(millon(numero % 1000000));
		}
	...
	private String milmillon(double numero) {
		numero = (long)(numero);
		if (numero == 1000000000)
			num_letradmm = "mil millones";
		if (numero > 1000000000 && numero < new Long("10000000000")) {
			flag = 1;
			num_letradmm = miles(numero / 1000000).concat("millones ").concat(cienmiles(numero % 1000000));
		}
		if (numero >= 2000000000 && numero <= 2147483647) {
			flag = 1;
			num_letradmm = miles(numero / 1000000).concat(" millones ").concat(millon(numero % 1000000));
		}

		if (numero < 1000000000)
			num_letradmm = cenmillon(numero);

		return num_letradmm;
	}

	public String convertirLetras(double numero) {
		num_letras = milmillon(numero);
		return num_letras;
	}
	/*public static void main(String[] args) {
		n2tDouble n = new n2tDouble();
		double num=958996999 ;
		System.out.println(n.convertirLetras(num));
		double i = "2".compareTo("a");
		if (i == 0) {
			//Son iguales
		}
		if (i < 0) {
